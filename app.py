...
# ==========================================
st.set_page_config(page_title="🚀 Pro AI Trader V4.7 (Radar Mode)", layout="wide", page_icon="🤖")

# Arka planda taranacak 20 popüler coin (Hafıza listesi)
BACKGROUND_RADAR_COINS = [
    "BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT", 
    "ADAUSDT", "DOGEUSDT", "AVAXUSDT", "LINKUSDT", "DOTUSDT",
    "MATICUSDT", "LTCUSDT", "SHIBUSDT", "TRXUSDT", "BCHUSDT",
...
def hard_reset_database():
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("UPDATE wallet SET balance=1000.0, start_balance=1000.0 WHERE id=1")
    c.execute("DELETE FROM positions")
    ...
    st.session_state.trade_history = pd.DataFrame(columns=["Tarih", "Coin", "İşlem", "PnL", "Sebep", "Snowball"])
...
def init_system_state():
    ...
    if "is_running" not in st.session_state:
        ...
        st.session_state.market_prices = {coin: 0.0 for coin in BACKGROUND_RADAR_COINS}
        st.session_state.sentiment_data = {coin: 0.0 for coin in BACKGROUND_RADAR_COINS}
...
# ==========================================
# 4. MARKET ENGINE (MULTI-EXCHANGE)
# ==========================================
def fetch_live_market_data(coins: list):
    ...
    # Hızlı tarama için session kullanıyoruz
    with requests.Session() as session:
        for coin in coins:
            success = False
            try:
                res = session.get(f"https://api.mexc.com/api/v3/ticker/price?symbol={coin}", headers=headers, timeout=2)
                if res.status_code == 200:
                    st.session_state.market_prices[coin] = float(res.json()['price'])
                    success = True
            except:
                pass
            ...
            if not success:
                try:
                    res = session.get(f"https://api.binance.com/api/v3/ticker/price?symbol={coin}", headers=headers, timeout=2)
                    if res.status_code == 200:
                        st.session_state.market_prices[coin] = float(res.json()['price'])
                except:
                    pass
    ...
    try:
        response = requests.get("https://api.alternative.me/fng/", headers=headers, timeout=3)
...
# ==========================================
# 5. STRATEGY ENGINE (SADECE SİNYALLERİ GÖSTER)
# ==========================================
...
def generate_kelly_plan(selected_coins: list, risk_multiplier: float) -> pd.DataFrame:
    ...
    for coin in selected_coins:
        ...
        # SADECE KELLY POZİTİF İSE (SİNYAL VARSA) LİSTEYE EKLE
        if kelly_fraction > 0:
    ...
    # Sinyal verenleri en güçlüden en zayıfa sırala
...
def execute_new_trades(plan_df: pd.DataFrame, daily_profit: float, settings: dict, is_backtest=False):
    # Ayarlanan maksimum açık işlem sınırını kontrol et
    if plan_df.empty or len(st.session_state.open_positions) >= settings["max_positions"] or st.session_state.balance < 50:
        return 
    ...
    if any(p["coin"] == coin for p in st.session_state.open_positions):
        return
...
def run_backtest(settings, days):
    ...
    for i in range(iterations):
        for coin in BACKGROUND_RADAR_COINS:
        ...
        plan = generate_kelly_plan(BACKGROUND_RADAR_COINS, settings["risk_mult"])
...
# ==========================================
def main():
    st.title("🚀 Pro AI Trader V4.7 (Görünmez Radar Modu)")
    init_system_state()
    ...
    with st.sidebar:
        ...
        # Kullanıcı artık coin seçmiyor, sadece aynı anda kaç işleme gireceğini seçiyor.
        settings = {
            "max_positions": st.number_input("Maksimum Eşzamanlı İşlem:", 1, 10, 4),
        ...
        st.info(f"📡 Arka planda anlık taranan coin sayısı: {len(BACKGROUND_RADAR_COINS)}")
        ...
        if app_mode == "🔴 Canlı İşlem":
            ...
            st.markdown("---")
            if st.button("🔄 Sistemi Sıfırla ($1000)", use_container_width=True):
                hard_reset_database()
                st.success("Sistem başarıyla 1000 dolara sıfırlandı!")
                time.sleep(1)
                st.rerun()
    ...
    if app_mode == "🔴 Canlı İşlem":
        ...
        with col_left:
            ...
            tbl_open = st.empty()
            st.subheader("🎯 Aktif Sinyaller (Radar Matrisi)")
            tbl_ai = st.empty()
        with col_right:
            st.subheader("📈 Sinyal / Açık İşlem Grafikleri")
            chart_plt = st.empty()
        ...
        if st.session_state.is_panic:
            if len(st.session_state.open_positions) > 0:
                for pos in st.session_state.open_positions:
                    curr_price = st.session_state.market_prices.get(pos["coin"], pos["entry"])
                    pnl = (curr_price - pos["entry"]) * pos["size"]
                    st.session_state.balance += pos["margin"] + pnl
            ...
            while st.session_state.is_running:
                # Arka planda 20 coini sessizce tara
                fetch_live_market_data(BACKGROUND_RADAR_COINS)
                
                manage_open_positions()
                st.session_state.ai_plan = generate_kelly_plan(BACKGROUND_RADAR_COINS, settings["risk_mult"])
                daily_profit = st.session_state.balance - st.session_state.start_balance
                ...
                if st.session_state.open_positions:
                    ...
                    tbl_open.info("Açık işlem yok. Radar sinyal arıyor...")
                    
                # Aktif Sinyaller Tablosu (Sadece Sinyal Varsa Göster)
                if not st.session_state.ai_plan.empty:
                    ...
                    tbl_ai.success("📡 Radar 20 coini tarıyor... Şu an hiçbir varlıkta matematiksel avantaj (Edge) yok.")
                ...
                # Grafikte sadece aktif sinyal olan veya açık işlemi olan coinleri göster
                fig = go.Figure()
                active_chart_coins = list(set([p["coin"] for p in st.session_state.open_positions] + 
                                              (st.session_state.ai_plan["Coin"].tolist() if not st.session_state.ai_plan.empty else [])))
                
                for coin in active_chart_coins:
                    base_price = st.session_state.market_prices.get(coin, 0.0)
                    if base_price > 0:
