...
# ==========================================
st.set_page_config(page_title="🚀 Pro AI Trader V4.5", layout="wide", page_icon="🤖")
...
def hard_reset_database():
    """Tüm veritabanını temizler ve kasayı 1000 dolara eşitler."""
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
...
# ==========================================
# 4. MARKET ENGINE (MULTI-EXCHANGE ANTI-BAN)
# ==========================================
def fetch_live_market_data(coins: list):
    ...
    for coin in coins:
        success = False
        try:
            res = requests.get(f"https://api.mexc.com/api/v3/ticker/price?symbol={coin}", headers=headers, timeout=3)
        ...
        if not success:
            try:
                res = requests.get(f"https://api.binance.com/api/v3/ticker/price?symbol={coin}", headers=headers, timeout=3)
                if res.status_code == 200:
                    st.session_state.market_prices[coin] = float(res.json()['price'])
            except:
                pass
...
# ==========================================
def main():
    st.title("🚀 Pro AI Trader V4.5 (Live & Reset Supported)")
    init_system_state()
        ...
        if st.session_state.is_panic:
            ...
            while st.session_state.is_running:
                ...
                if any(st.session_state.market_prices.get(coin, 0.0) == 0.0 for coin in settings["coins"]):
                    st.warning("⏳ Borsalara bağlanılıyor... Gerçek veri gelene kadar işlem açılmayacak.")
                    time.sleep(2)
                    st.rerun()
