...
# ==========================================
st.set_page_config(page_title="🚀 Pro AI Trader V4", layout="wide", page_icon="🤖")
...
def init_system_state():
    ...
    if "is_running" not in st.session_state:
        ...
        st.session_state.market_prices = {"BTCUSDT": 64000.0, "ETHUSDT": 3400.0, "SOLUSDT": 145.0, "ADAUSDT": 0.45}
...
def generate_kelly_plan(selected_coins: list, risk_multiplier: float) -> pd.DataFrame:
    ...
    for coin in selected_coins:
        base_price = st.session_state.market_prices[coin]
        
        # --- MTF (Multi-Timeframe) Simülasyonu ---
        # 4 Saatlik (Makro) Grafik Simülasyonu
        ...
        # 15 Dakikalık (Mikro) Grafik Simülasyonu
        ...
        # MTF Karar Mantığı (Fakeout Koruması)
        ...
        if macro_dir == 1 and micro_dir == 1:
            ...
            p_technical = np.random.uniform(0.20, 0.35) # Long açma ihtimalini öldür
        elif macro_dir == -1 and micro_dir == 1:
            mtf_status = "Boğa Tuzağı (Fakeout) ⚠️"
            p_technical = np.random.uniform(0.35, 0.45) # Tuzağa düşmemek için olasılığı düşür
        elif macro_dir == 1 and micro_dir == -1:
            mtf_status = "Düzeltme (Pullback) ⏳"
            p_technical = np.random.uniform(0.50, 0.60) # Alım fırsatı olabilir
        ...
        sentiment = st.session_state.sentiment_data[coin]
        p = max(0.1, min(0.95, p_technical + (sentiment * 0.15))) # Sentiment etkisi
    ...
    return pd.DataFrame(plan_data).sort_values(by="Margin Pct", ascending=False)
...
def manage_open_positions(is_backtest=False):
    ...
    for pos in st.session_state.open_positions:
        curr_price = st.session_state.market_prices[pos["coin"]]
...
def execute_new_trades(plan_df: pd.DataFrame, daily_profit: float, settings: dict, is_backtest=False):
    if len(st.session_state.open_positions) >= len(settings["coins"]) or st.session_state.balance < 50:
        return 
...
# ==========================================
# 7. BACKTEST ENGINE (YENİ)
# ==========================================
def run_backtest(settings, days):
    """Geçmişe dönük hızlı simülasyon motoru"""
    bt_balance = 1000.0
    bt_start_balance = 1000.0
    st.session_state.open_positions = [] # Backtest için pozisyonları sıfırla
    st.session_state.balance = bt_balance
    
    iterations = days * 24 # Saatlik mum simülasyonu
    winning_trades = 0
    total_trades = 0
    max_drawdown = 0.0
    ...
    for i in range(iterations):
        # 1. Sahte Fiyat Hareketi
        for coin in settings["coins"]:
            st.session_state.market_prices[coin] *= (1 + np.random.normal(0.0002, 0.005))
            st.session_state.sentiment_data[coin] = np.random.normal(0, 0.3)
            
        # 2. Portföy Yönetimi
        manage_open_positions(is_backtest=True)
        
        # 3. AI Plan & Yeni İşlemler
    ...
    st.session_state.open_positions = [] # Temizle
    ...
    # Cüzdanı eski DB haline geri getir
    init_system_state()
...
# ==========================================
def main():
    st.title("🚀 Pro AI Trader V4 (MTF + Backtest)")
    init_system_state()
    ...
    if app_mode == "🔴 Canlı İşlem":
        ...
        if st.session_state.is_panic:
            ...
            while st.session_state.is_running:
                ...
                fig = go.Figure()
                for coin in settings["coins"]:
                    hist = [st.session_state.market_prices[coin] * (1 + np.random.normal(0, 0.005)) for _ in range(15)]
