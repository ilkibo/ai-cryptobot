...
# ==========================================
st.set_page_config(page_title="🚀 Pro AI Trader V4.2", layout="wide", page_icon="🤖")
...
# ==========================================
# 4. MARKET ENGINE (Anti-Ban YENİ)
# ==========================================
def fetch_live_market_data(coins: list):
    # 1. Aşama: Binance Doğrudan REST API (Çok Hızlı)
    for coin in coins:
        try:
    ...
    # 2. Aşama: Fallback (Eğer Binance Cloud IP'sini banladıysa sistem kilitlenmesin)
    fallbacks = {"BTCUSDT": 65000.0, "ETHUSDT": 3500.0, "SOLUSDT": 150.0, "ADAUSDT": 0.50}
    for coin in coins:
        if st.session_state.market_prices.get(coin, 0.0) == 0.0:
            st.session_state.market_prices[coin] = fallbacks.get(coin, 100.0)
    ...
    try:
        ...
        pass # Sentiment çekilemezse nötr (0.0) kalır
...
# ==========================================
def main():
    st.title("🚀 Pro AI Trader V4.2 (Anti-Ban Live)")
    init_system_state()
        ...
        if st.session_state.is_panic:
            ...
            while st.session_state.is_running:
                ...
                if any(st.session_state.market_prices.get(coin, 0.0) == 0.0 for coin in settings["coins"]):
                    st.warning("⏳ Sistem başlatılıyor... Veri bekleniyor.")
                    time.sleep(1)
                    st.rerun()
                # -------------------------
                ...
                time.sleep(2) # Streamlit sunucusunu yormamak için 2 saniye bekle
                st.rerun()
        else:
