...
# ==========================================
st.set_page_config(page_title="🚀 Pro AI Trader V4.3", layout="wide", page_icon="🤖")
...
# ==========================================
# 4. MARKET ENGINE (ULTIMATE ANTI-BAN)
# ==========================================
def fetch_live_market_data(coins: list):
    # Binance bizi "Bot" sanmasın diye sahte Google Chrome kimliği (User-Agent) ekliyoruz.
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36',
        'Accept': 'application/json'
    }
    
    # Binance'in farklı sunucularını denemek için havuz oluşturduk
    api_endpoints = [
        "https://api.binance.com/api/v3/ticker/price?symbol=",
        "https://api1.binance.com/api/v3/ticker/price?symbol=",
        "https://api2.binance.com/api/v3/ticker/price?symbol=",
    ...
    for coin in coins:
        ...
        # 1. Aşama: REST API Bypass
        for base_url in api_endpoints:
            try:
                res = requests.get(base_url + coin, headers=headers, timeout=3)
                if res.status_code == 200:
                    st.session_state.market_prices[coin] = float(res.json()['price'])
                    success = True
                    break # Başarılıysa diğer sunucuyu denemeyi bırak
            except:
                continue
                
        # 2. Aşama: CCXT ile Yedek Bağlantı (REST API çalışmazsa devreye girer)
        if not success:
            try:
                exchange = ccxt.binance({'enableRateLimit': True})
                ticker = exchange.fetch_ticker(coin.replace("USDT", "/USDT"))
                st.session_state.market_prices[coin] = float(ticker['last'])
                success = True
            except:
                pass

    # 3. Aşama: Haber Duyarlılığı (Sentiment)
    try:
...
# ==========================================
def main():
    st.title("🚀 Pro AI Trader V4.3 (Anti-Ban Live)")
    init_system_state()
    ...
    if app_mode == "🔴 Canlı İşlem":
        ...
        if st.session_state.is_panic:
            if len(st.session_state.open_positions) > 0:
                for pos in st.session_state.open_positions:
                    # Anlık fiyat sıfırsa hata vermemesi için koruma
            ...
            while st.session_state.is_running:
                ...
                # --- VERİ KİLİDİ VE UYARI ---
                if any(st.session_state.market_prices.get(coin, 0.0) == 0.0 for coin in settings["coins"]):
                    st.warning("⏳ Binance canlı verisi bekleniyor veya engellendi... Sahte veri ile işlem açılmayacak.")
                    time.sleep(1)
                    st.rerun()
                # -----------------------------
                ...
                if st.session_state.open_positions:
                    ...
                    tbl_open.info("MTF onayı ve Matematiksel Edge bekleniyor...")
